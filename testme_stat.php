...
//Проверяем, есть ли строка с таким ИД
        $testme_query = "SELECT t.ID, test_name, DATE_FORMAT(test_start_day,GET_FORMAT(DATE,'EUR')) AS test_start_day, test_user, 
            test_status, test_post, display_name, post_status, test_done, p.comment_count
...
//Если нет такого теста
        if (!isset($testme_test_stat->ID) || ($testme_test_stat->test_user != $current_user->ID && !current_user_can($testme_user_level) )) {
...
// А если есть, то выводим статистику
            ...
            if ($testme_test_stat->post_status == 'publish') {
                print '<a href="' . get_permalink($testme_test_stat->test_post) . '" target="_blank">Запись с тестом</a>.<br />';
            } else {
            ...
            if ($testme_test_stat->comment_count > 0) {
                $testme_comment_count = $testme_test_stat->comment_count;
            } else {
                $testme_comment_count = 0;
            }
...
// Статистика прохождений
        if (get_option('testme_stat_allow') == 'yes') {
            ...
            $testme_stat_done = $wpdb->get_var("SELECT COUNT(Id) 
                FROM " . $wpdb->testme_stats . " 
                WHERE stat_test_relation = " . $testme_id . "");

            //Получаем данные по ответам:
            $testme_stats = $wpdb->get_results(" SELECT result_title, result_text, result_image , COUNT(st.Id) AS NUM
FROM " . $wpdb->testme_results . " AS res
LEFT JOIN " . $wpdb->testme_stats . " AS st ON res.Id = st.stat_result
WHERE res.result_test_relation = " . $testme_id . "
GROUP BY res.Id
ORDER BY res.Id");
                        ...
                        foreach ($testme_stats as $stat) {
                            ...
                            if ($stat->result_title != '') {
                                ...
                                if (function_exists(mb_strtolower)) {
                                    $testme_title = mb_substr(strip_tags(stripslashes($stat->result_text, 0, 70))) . " ...";
                                } else {
                                    $testme_title = substr(strip_tags(stripslashes($stat->result_text, 0, 70))) . " ...";
                                }
                            } else {
...
// Получаем N последних прошедших
                        $testme_limit = get_option("testme_stat_per_page", 10);

                        $testme_stats_2 = $wpdb->get_results(" 
SELECT stat_time, stat_result, stat_user, stat_ip, stat_points, display_name
FROM " . $wpdb->testme_stats . " AS st
LEFT JOIN " . $wpdb->users . " AS u ON st.stat_user = u.ID
WHERE st.stat_test_relation = " . $testme_id . "
