    ...
    <div class="postbox" id="testme_results_div">
        ...
        <div class="inside">
            ...
            <p>Тип: <?php print $testme_test_details->test_type ?><br />
                Только зарегистрированные пользователи могут проходить этот тест: <?php
                if ($testme_test_details->test_only_reg == 1) {
                    print "Да";
                } else {
                    print "Нет";
                }
                ?><br />
                Выводить вопросы теста в случайном порядке: <?php
                ...
                ?><br />
                Выводить ответы к вопросам в случайном порядке: <?php
                ...
                ?><br />
                <?php if ($testme_test_details->test_type == '123') { ?>Показывать пользователю, сколько он набрал баллов в тесте: <?php
            ...
            <div class="testme_4_desc">
                <?php print stripslashes($testme_test_details->test_description) ?>
            </div>
            ...
            <?php
            //Список вопросов
            $testme_questions = $wpdb->get_results("SELECT ID, question_text FROM " . $wpdb->testme_questions . " WHERE question_test_relation= {$testme_id} ORDER BY ID");
            ...
            if ($testme_questions) {
                ...
                foreach ($testme_questions as $ques) {
                    ...
                    $testme_dans = $wpdb->get_results("SELECT ID, answer_text, answer_points FROM {$wpdb->testme_answers} WHERE answer_question_relation={$ques->ID} ORDER BY ID");
            ...
            <h3>Результаты:</h3>
            <?php
            include (WP_PLUGIN_DIR . '/testme/testme_edit_one_res.php');

            //Список результатов
            $testme_results = $wpdb->get_results(" SELECT * FROM " . $wpdb->testme_results . "
            ...
            if ($testme_results) {
                ...
                foreach ($testme_results as $result) {
                    ...
                    if ($result->result_title != '') {
                        print '<h4>' . $result->result_title . '</h4>';
                    }
                    if ($result->result_image != '') {
                        print '<img src="' . $result->result_image . '" class="testme_' . $result->result_image_position . '" alt="' . $result->result_title . '" />';
                    }
                    if ($result->result_text != '') {
                        print "<p>" . nl2br(stripslashes($result->result_text)) . "</p>";
                    }
            ...
            <?php
// Кнопка Тест готов для авторов теста при статусе 1 и 3
            if ($testme_test_details->test_status == 1 && ($testme_test_details->test_user == $current_user->ID || current_user_can($testme_user_level))) {
                ...
                if ($testme_questions && $testme_results) {
                    ...
                    <script type="text/javascript">
                        ...
                        var testme_ajaxurl = '<?php echo admin_url('admin-ajax.php', (is_ssl() ? 'https' : 'http')); ?>';
                                $("#testme_step4_action_area").html('<img src="<?php echo plugins_url('testme/images/loading4.gif') ?>" alt="" />');
                                jQuery.ajax({
                                url: testme_ajaxurl,
                                        method: 'GET',
                                        data: 'action=testme_adm&task=testready&testme_id=<?php echo $testme_id ?>',
